...
#include "version.h"
#include "../../qdmautils/qdmautils.h"
#include "../../include/qdma_nl.h"
#include "../../qdmautils/dmaxfer.h"
...
static inline void qdma_q_prep_name(struct queue_info *q_info, int qid, int pf)
{
	...
	snprintf(q_info->q_name, QDMA_Q_NAME_LEN, "/dev/qdma%s%05x-%s-%u",
			(is_vf) ? "vf" : "",
...
static int qdma_register_read(unsigned int pf, int bar,
		unsigned long reg, unsigned long *value)
{
	struct xcmd_info xcmd;
	struct xcmd_reg *regcmd;
		...
		char *output_file, int io_type)
{
	...
	if (io_type == 0) {
		...
		if (ret < 0)
			...
			printf("PF :%u Queue :%u C2H Sync transfer success\n", q_info->pf, q_info->qid); 
	} else {
		...
		if (ret < 0)
			...
			printf("PF :%u Queue :%u C2H ASync transfer success\n", q_info->pf, q_info->qid); 
	}
	if (ret < 0)
		goto out;
		...
		char *input_file, int io_type)
{
	...
	if (io_type == 0) {
		...
		if (ret < 0)
			...
			printf("PF :%u Queue :%u H2C Sync transfer success\n", q_info->pf, q_info->qid); 
	} else {
		ret = qdmautils_async_xfer(q_info->q_name,
				q_info->dir, buffer, size);
		if (ret < 0)
			...
			printf("PF :%u Queue :%u H2C Async transfer success\n", q_info->pf, q_info->qid); 
	}
