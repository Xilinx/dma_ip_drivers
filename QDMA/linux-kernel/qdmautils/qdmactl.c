		       ...
		       uint32_t *attrs)
{
	...
	if (attrs && attrs[XNL_ATTR_ERROR] != 0)
...
static void xnl_parse_csr_attrs(uint32_t *attrs, struct xcmd_info *xcmd)
{
	memcpy(&xcmd->resp.csr, &attrs[XNL_ATTR_GLOBAL_CSR],
				...
				uint32_t *attrs)
{
	...
	switch(xcmd->op) {
        ...
        case XNL_CMD_GLOBAL_CSR:
        	xnl_parse_csr_attrs(attrs, xcmd);
		break;
	default:
		break;
	}
