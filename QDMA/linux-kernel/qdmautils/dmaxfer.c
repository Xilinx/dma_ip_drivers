...
static void *io_thread(struct io_info *_info)
{
	...
	do {
		...
		if (tsecs) {
			...
			timespec_sub(&ts_cur, &_info->g_ts_start);
			...
			unsigned int num_entries)
{
	...
	if ((handle->shmid <= 0) || (handle->active_threads <= 0))
		return;
	shmid = handle->shmid;
